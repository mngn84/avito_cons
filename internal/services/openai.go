...
func (s *openaiService) GetResponse(text string, chatId string, userId int, created int, itemInfo avito_models.Value) (string, error) {
	...
	promt := fmt.Sprintf("%sProductInCurrentChat:%sPrice:%s",
		s.config.OpenAI.SystemPrompt,
		itemInfo.Title,
		itemInfo.PriceString,
	)
	s.logger.Info("promt", "promt", promt)
	...
	s.logger.Info("messages", "messages", messages)
